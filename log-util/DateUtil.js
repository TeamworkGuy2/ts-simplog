...
(function (DateUtil) {
    ...
    function getWeekInYear(date, minimalDaysInFirstWeek) {
        ...
        var weeksSinceSartOfYear = Math.floor(getTimeSince(previousSunday, startOfYear) / DateUtil.MILLIS_PER_WEEK);
    ...
    function getWeekInMonth(date, minimalDaysInFirstWeek) {
        ...
        var numberOfSundays = isBefore(previousSunday, startOfMonth) ?
            0 : 1 + Math.floor(getTimeSince(previousSunday, startOfMonth) / DateUtil.MILLIS_PER_WEEK);
    ...
    function getDayInYear(date) {
        var startOfYear = newMidnightDate(date.getFullYear(), 0, 1);
        return 1 + Math.floor(getTimeSince(date, startOfYear) / DateUtil.MILLIS_PER_DAY);
    }
